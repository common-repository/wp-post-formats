...
function wppf_save_data(){
    ...
    wp_enqueue_script( 'ace-js'       , WPPF_ABSPATH . '/lib/ace/ace.js' );
    wp_enqueue_script( 'wppf-admin-js', WPPF_ABSPATH . '/admin-js.js'    );
...
/**
 * Sets Some default values for the plugin options
 */
...
function wppf_safe_format( $format , $post_id ) {
	...
	foreach( $datarr as $key => $val ) :
		 ...
		 */
		if( strpos( $val , 'post-thumbnail' ) === 0 && current_theme_supports( 'post-thumbnails' ) ) {
		...
		/**
         * Allow plugins and themes to add there own tags.
		 */
		do_action( 'wppf_tags' );
